...
public class MovieProvider extends ContentProvider {
    private static final int MOVIE = 1001;
        ...
        public String getType(Uri uri) {
            ...
            switch (match) {
                case MOVIE: {
                    return MovieDatabase.MovieEntry.CONTENT_TYPE;
                }

                case MOVIE_WITH_ID: {
                    return MovieDatabase.MovieEntry.CONTENT_ITEM_TYPE;
                }
        ...
        public Uri insert(Uri uri, ContentValues values) {
            ...
            switch (match) {
                case MOVIE: {
                    long id = sqLiteDatabase.insert(MovieDatabase.Tables.MOVIE, null, values);
                    if (id == -1) {
                        sqLiteDatabase.close();
                        throw new SQLException("Failed to insert row into " + uri);
                    }
                    Uri returnUri = MovieDatabase.MovieEntry.buildMovieUri(id);
                    notifyChange(uri);
                /*
         ...
         */
            ...
            switch (match) {
                case MOVIE: {
                    rowsDeleted = sqLiteDatabase.delete(MovieDatabase.Tables.MOVIE, selection, selectionArgs);
                    break;
                }
        ...
        private static UriMatcher buildUriMatcher() {
            ...
            uriMatcher.addURI(MovieDatabase.CONTENT_AUTHORITY, MovieDatabase.PATH_MOVIE, MOVIE);

            //Match uri with movie id
            uriMatcher.addURI(MovieDatabase.CONTENT_AUTHORITY, MovieDatabase.PATH_MOVIE + "/#", MOVIE_WITH_ID);
